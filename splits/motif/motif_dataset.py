import sys
from splits.motif.utils.best_split_utils import *
import pandas as pd
import json
import os
...
class MotifDataset:
    def __init__(self, motif_dataset_jsonl_filename):
        ...
        samples_per_family_count = samples_per_family_count[
            samples_per_family_count >= 10
        ]
        ...
        assert df_motif_train.shape[0] + df_motif_test.shape[0] == df_motif.shape[0], (
            "Train and test sets do not match the original dataset size"
        )

        train_family_freq = df_motif_train["family"].value_counts()
        train_family_freq = train_family_freq[train_family_freq > 3]

        df_motif_train = df_motif_train[
            df_motif_train["family"].isin(train_family_freq.index)
        ]
